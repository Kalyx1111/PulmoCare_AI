...
call :LOG "===== PulmoCare AI Launch Started %DATE% %TIME% ====="
cls
echo.
echo  ================================================================
echo   PulmoCare AI  -  Pulmonology Health Intelligence v1.0
echo  ================================================================
echo.
echo  MEDICAL DISCLAIMER:
echo  This is an AI Research Tool ONLY. Not medical advice.
echo  ALWAYS consult your pulmonologist before any treatment decision.
echo  PULMONARY EMERGENCY: 112 (India) / 999 (UK) / 911 (US)
echo  Severe breathlessness / blue lips: CALL EMERGENCY SERVICES
echo.
echo  ================================================================
echo.
...
echo.
echo  [STEP 6/7]  Checking port 5110...
call :LOG "Checking port 5110"
netstat -ano 2>nul | findstr ":5110 " >nul 2>&1
if %errorlevel%==0 (for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5110 "') do taskkill /PID %%P /F >nul 2>&1 & timeout /t 2 /nobreak >nul & echo              [ OK ]  Port freed.) else (echo              [ OK ]  Port 5110 available.)
echo.
echo  [STEP 7/7]  Starting PulmoCare AI...
call :LOG "Starting server on port 5110"
...
start "PulmoCare AI Server" cmd /k "echo PulmoCare AI Server - Keep This Window Open && "%VPYTHON%" "%BASEDIR%\server.py" --port 5110"
echo              Waiting for server...
set "SERVER_UP=false"
for /L %%i in (1,1,25) do (
    timeout /t 1 /nobreak >nul
    curl -s "http://localhost:5110/api/health" >nul 2>&1
...
echo.
echo  ================================================================
echo   PulmoCare AI IS RUNNING
echo  ================================================================
echo.
echo   Open in browser:   http://localhost:5110
...
goto :MENU
:QUIT
taskkill /FI "WINDOWTITLE eq PulmoCare AI Server*" /F >nul 2>&1
for /f "tokens=5" %%P in ('netstat -ano 2^>nul ^| findstr ":5110 "') do taskkill /PID %%P /F >nul 2>&1
